# utils/helpers.py
# Helper functions for user management, game stats, and utilities
...
from config import (
    ADMIN_ID, ADMIN_BALANCE, BOT_USERNAME, GAME_TYPES, RANKS,
    STARS_TO_USD, STARS_TO_TON
)
...
def get_user_balance(user_id: int) -> float:
    """Get user's current balance. Admins have unlimited balance."""
    if is_admin(user_id):
...
def adjust_user_balance(user_id: int, amount: float) -> None:
    """Adjust user's balance by adding/subtracting amount."""
    if not is_admin(user_id):
        db.user_balances[user_id] += amount
        db.save_data()
...
def get_user_link(user_id: int, name: str) -> str:
    """Generate HTML link to user profile."""
...
def get_user_rank(xp: int) -> int:
    """Calculate user's rank level based on XP."""
    current_rank = 1
