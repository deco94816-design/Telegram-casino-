# database.py
# Data persistence and storage management for the casino bot
...
from config import DATA_FILE, ADMIN_ID, INITIAL_WITHDRAWAL_COUNTER, logger

# ==================== DATA STRUCTURES ====================

# User balances (user_id -> balance)
user_balances: Dict[int, float] = defaultdict(float)

# User profiles (user_id -> profile dict)
user_profiles: Dict[int, Dict[str, Any]] = {}

# User game history (user_id -> list of game records)
user_game_history: Dict[int, List[Dict[str, Any]]] = defaultdict(list)

# Active games (user_id -> Game object)
user_games: Dict[int, Any] = {}

# Game locks for thread safety
game_locks: Dict[int, asyncio.Lock] = defaultdict(asyncio.Lock)

# Users who have claimed bonus
user_bonus_claimed: Set[int] = set()

# Last game settings for repeat/double feature
user_last_game_settings: Dict[int, Dict[str, Any]] = {}

# Username to user_id mapping
username_to_id: Dict[str, int] = {}

# User withdrawals
user_withdrawals: Dict[str, Dict[str, Any]] = {}

# Withdrawal counter
withdrawal_counter: int = INITIAL_WITHDRAWAL_COUNTER

# Admin list
admin_list: Set[int] = {ADMIN_ID}

# Withdraw video file_id (set by admin via /video command)
withdraw_video_file_id: Optional[str] = None
...
def load_data() -> None:
    ...
    try:
        ...
        withdrawal_counter = data.get('withdrawal_counter', INITIAL_WITHDRAWAL_COUNTER)
        admin_list.update(set(data.get('admin_list', [ADMIN_ID])))
        username_to_id.update(data.get('username_to_id', {}))
        user_last_game_settings.update({
            int(k): v for k, v in data.get('user_last_game_settings', {}).items()
        })
...
def get_withdrawal_counter() -> int:
    """Get current withdrawal counter."""
    return withdrawal_counter


def increment_withdrawal_counter() -> int:
    """Increment and return withdrawal counter."""
...
def set_withdraw_video(file_id: Optional[str]) -> None:
...
def get_withdraw_video() -> Optional[str]:
