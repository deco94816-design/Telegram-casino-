# handlers/basic.py
# Basic command handlers: /start, /help, /balance, /profile, /history

from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import ContextTypes

from config import STARS_TO_USD, GAME_TYPES, logger
from database import user_profiles, user_game_history, username_to_id, save_data
from utils.decorators import handle_errors
from utils.helpers import (
    is_admin, get_user_balance, get_or_create_profile, get_user_rank,
    get_rank_info, get_user_link, create_progress_bar
)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        f"🎁 <b>Bonus:</b>\n"
        f"Add 'Iibrate' to your profile name and use /bonus to get 20 ⭐!\n\n"
        "📝 <b>Command Aliases:</b>\n"
        "• /bal = /balance\n"
        "• /depo = /deposit\n\n"
    )
...
async def balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /balance and /bal commands - Display user's balance."""
    user_id = update.effective_user.id
    balance = get_user_balance(user_id)
    balance_usd = balance * STARS_TO_USD
...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /profile command - Display user's profile and stats."""
    user = update.effective_user
    user_id = user.id
...
async def history_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /history command - Display user's game history and statistics."""
    user = update.effective_user
    user_id = user.id
