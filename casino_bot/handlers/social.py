...
from config import BOT_USERNAME, BONUS_AMOUNT, STARS_TO_USD, logger
from utils.decorators import handle_errors
from utils.helpers import (
    is_admin, get_user_balance, get_user_link, get_or_create_profile,
    check_bot_name_in_profile, get_user_id_by_username, adjust_user_balance
)
...
async def tip_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /tip command - send tips to other users."""
    ...
    if context.args and len(context.args) >= 2:
        try:
            ...
            logger.info(
                f"Tip: {user_id} ({sender_name}) -> {recipient_id} ({recipient_name}): {tip_amount} stars"
            )
            return
