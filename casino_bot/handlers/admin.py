# handlers/admin.py
# Admin command handlers: /addadmin, /removeadmin, /listadmins, /video

from telegram import Update
from telegram.ext import ContextTypes

from config import ADMIN_ID, ADMIN_BALANCE, logger
...
async def addadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /addadmin command - add a new admin."""
    ...
    try:
        ...
        if new_admin_id in db.admin_list:
            await update.message.reply_html(
                f"⚠️ User <code>{new_admin_id}</code> is already an admin!"
            )
            return
...
async def removeadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /removeadmin command - remove an admin."""
    user_id = update.effective_user.id
    ...
    try:
        ...
        if remove_admin_id not in db.admin_list:
            await update.message.reply_html(
                f"⚠️ User <code>{remove_admin_id}</code> is not an admin!"
            )
            return
...
@handle_errors
async def set_video_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /video command - admin command to set the withdraw video."""
    user_id = update.effective_user.id
...
async def handle_video_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get video from message (can be video or animation/GIF or document)
