# handlers/admin.py
# Admin command handlers: /addadmin, /removeadmin, /listadmins, /video, /cancel
...
async def addadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /addadmin command - Add new admin (admin only)."""
...
async def removeadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /removeadmin command - Remove an admin (admin only)."""
    user_id = update.effective_user.id
...
async def listadmins_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /listadmins command - List all admins (admin only)."""
    user_id = update.effective_user.id
...
async def set_video_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].lower() == 'status':
        withdraw_video_file_id = db.get_withdraw_video()
