...
from database import (
    admin_list, user_balances, save_data,
    get_withdraw_video_file_id, set_withdraw_video_file_id
)
...
async def set_video_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /video command - Set the withdraw video (admin only)."""
    user_id = update.effective_user.id
    ...
    if context.args and context.args[0].lower() == 'remove':
        if withdraw_video_file_id:
            set_withdraw_video_file_id(None)
            await update.message.reply_html(
...
async def handle_video_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    set_withdraw_video_file_id(video.file_id)
