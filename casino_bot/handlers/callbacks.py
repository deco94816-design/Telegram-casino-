...
from config import (
    GAME_TYPES, STARS_TO_USD, STARS_TO_TON, MIN_WITHDRAWAL, 
    PROVIDER_TOKEN, logger
)
from models import Game
from utils.decorators import handle_errors
from utils.helpers import (
    is_admin, get_user_balance, generate_transaction_id,
    save_last_game_settings, start_repeat_game
)
import database as db
from .games import start_game_from_callback
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if data == "balance_deposit":
            await _handle_balance_deposit(query)
            return
        
        if data == "balance_withdraw":
            await _handle_balance_withdraw(query, context)
            return
        
        if data == "back_to_balance":
            await _handle_back_to_balance(query, user_id)
            return
        
        if data == "show_games":
            await _handle_show_games(query)
            return
        ...
        if data == "start_withdraw":
            await _handle_start_withdraw(query, context)
            return
        
        if data == "confirm_withdraw":
            await _handle_confirm_withdraw(query, context, user_id)
            return
        
        if data == "cancel_withdraw":
            await _handle_cancel_withdraw(query, context)
            return
        
        if data.startswith("deposit_"):
            await _handle_deposit(query, context, data)
            return
        
        if data.startswith("demo_game_"):
            await _handle_demo_game(query, context, user_id, data)
            return
        
        if data == "back_to_demo_menu":
            await _handle_back_to_demo_menu(query)
            return
        
        if data.startswith("demo_bet_"):
            await _handle_demo_bet(query, context, user_id, data)
            return
        
        if data.startswith("bet_"):
            await _handle_bet(query, context, user_id, data)
            return
        
        if data.startswith("back_to_bet_"):
            await _handle_back_to_bet(query, user_id, data)
            return
        
        if data.startswith("rounds_"):
            await _handle_rounds(query, context, data)
            return
        
        if data.startswith("throws_"):
            await _handle_throws(query, context, data)
            return
        
        if data.startswith("start_game_"):
            await _handle_start_game(query, context, user_id, data)
            return
        
        if data == "cancel_game":
            await _handle_cancel_game(query, user_id)
            return
...
async def _handle_balance_deposit(query) -> None:
...
async def _handle_balance_withdraw(query, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def _handle_back_to_balance(query, user_id: int) -> None:
    """Handle back to balance button."""
...
async def _handle_show_games(query) -> None:
    """Handle show games button."""
    keyboard = [
        [
...
async def _handle_start_withdraw(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle start withdraw button."""
...
async def _handle_confirm_withdraw(query, context: ContextTypes.DEFAULT_TYPE, 
                                   user_id: int) -> None:
    """Handle confirm withdraw button."""
...
async def _handle_cancel_withdraw(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle cancel withdraw button."""
    context.user_data['withdraw_state'] = None
    context.user_data['withdraw_amount'] = None
    context.user_data['withdraw_address'] = None
    await query.edit_message_text(
...
async def _handle_deposit(query, context: ContextTypes.DEFAULT_TYPE, data: str) -> None:
...
async def _handle_demo_game(query, context: ContextTypes.DEFAULT_TYPE, 
...
async def _handle_back_to_demo_menu(query) -> None:
    """Handle back to demo menu button."""
    keyboard = [
        [
...
async def _handle_demo_bet(query, context: ContextTypes.DEFAULT_TYPE, 
                          user_id: int, data: str) -> None:
...
async def _handle_bet(query, context: ContextTypes.DEFAULT_TYPE, 
                     user_id: int, data: str) -> None:
...
async def _handle_back_to_bet(query, user_id: int, data: str) -> None:
    """Handle back to bet selection button."""
...
async def _handle_rounds(query, context: ContextTypes.DEFAULT_TYPE, data: str) -> None:
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "Back ◀️", 
                callback_data=f"bet_{game_type}_{context.user_data.get('bet_amount', 10)}"
            ),
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def _handle_throws(query, context: ContextTypes.DEFAULT_TYPE, data: str) -> None:
    """Handle throws selection."""
    parts = data.split("_")
    game_type = parts[1]
    throws = int(parts[2])
...
async def _handle_start_game(query, context: ContextTypes.DEFAULT_TYPE, 
                            user_id: int, data: str) -> None:
    """Handle game start."""
    parts = data.split("_")
    game_type = parts[2]
...
async def _handle_cancel_game(query, user_id: int) -> None:
